...
if [ ! -f flutter_sound/pubspec.yaml ]
then
	echo "No file ./flutter_sound/pubspec.yaml"
	exit -1
fi
...
check_actual_flavor()
{
	...
	if grep '^name: flutter_sound_lite$' pubspec.yaml > /dev/null ; then
		...
		FLAVOR=FULL
		return 0
  elif grep '^name: flauto_lite$' pubspec.yaml > /dev/null ; then
    echo "Current flavor is LITE $(tput setaf 2) [OK]$(tput sgr 0)"
    FLAVOR=LITE
    return 0
  elif  grep '^name: flauto$' pubspec.yaml > /dev/null ; then
    echo "Current flavor is FULL $(tput setaf 2) [OK]$(tput sgr 0)"
    FLAVOR=FULL
    return 0
	else
...
check_android_java()
{
	RES=0
      	if [ $FLAVOR == 'FULL' ]; then
       		if grep "^ *import *com.dooboolab.ffmpeg.FlutterFFmpegPlugin;$" android/src/main/java/com/dooboolab/fluttersound/FlutterSound.java  > /dev/null; then
       		...
       		if grep 'public static *final *boolean *FULL_FLAVOR *= *true;$' android/src/main/java/com/dooboolab/fluttersound/FlutterSound.java  > /dev/null; then
      		...
      		if grep "^ *import *com.dooboolab.ffmpeg.FlutterFFmpegPlugin;$" android/src/main/java/com/dooboolab/fluttersound/FlutterSound.java   > /dev/null; then
       		...
       		if grep "public static *final *boolean *FULL_FLAVOR *= *false;$" android/src/main/java/com/dooboolab/fluttersound/FlutterSound.java  > /dev/null; then
       		...
       		if grep "^[ $'\t']*FlutterFFmpegPlugin.attachFFmpegPlugin *(ctx, *registrar.messenger ( *) *);$" android/src/main/java/com/dooboolab/fluttersound/FlutterSound.java  > /dev/null; then
...
FULL)
  ...
  cd flutter_sound
    ...
    gsed -i  's/^name: flauto_lite$/name: flauto/' pubspec.yaml
		...
		gsed -i  "/s.dependency *'mobile-ffmpeg-audio', *=* '4.3.LTS'$/d"   ios/flutter_sound.podspec
