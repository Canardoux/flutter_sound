...
function getScriptParser(parser, doc, block) {
...
function getEspreeFromUser() {
    try {
        ...
        const relativeTo = path__default.join(cwd, "__placeholder__.js");
...
class Parser {
    ...
    parse() {
        ...
        const parserOptions = Object.assign(Object.assign({}, this.baseParserOptions), { parser: getScriptParser(this.baseParserOptions.parser, this.document, "template") });
...
function getLang$1(customBlock) {
    var _a, _b;
                ...
                scriptVisitor["Program:exit"] = (node) => {
                    try {
                        ...
                        for (const customBlock of customBlocks) {
                            const lang = getLang$1(customBlock);
...
function parseScript$1(code, parserOptions, locationCalculator) {
    try {
...
function parseScriptSetupElements(scriptSetupElement, scriptElement, code, linesAndColumns, originalParserOptions) {
    ...
    const scriptSetupModuleCodeBlocks = getScriptSetupModuleCodeBlocks(scriptSetupElement, scriptElement, code, linesAndColumns, parserOptions);
    ...
    const result = parseScript$1(scriptSetupModuleCodeBlocks.codeBlocks.code, parserOptions, locationCalculator);
...
function getScriptSetupCodeBlocks(node, sfcCode, linesAndColumns, parserOptions) {
    ...
    const offsetLocationCalculator = simpleOffsetLocationCalculator(scriptSetupStartOffset, linesAndColumns);
    const result = parseScript$1(scriptCode, parserOptions, offsetLocationCalculator);
    const { ast } = result;
...
function parseForESLint(code, parserOptions) {
    ...
    if (!isVueFile(code, options)) {
        result = parseScript(code, Object.assign(Object.assign({}, options), { ecmaVersion: options.ecmaVersion || DEFAULT_ECMA_VERSION, parser: getScriptParser(options.parser, null, "script") }));
        document = null;
        ...
        if (skipParsingScript || !scripts.length) {
            result = parseScript("", Object.assign(Object.assign({}, options), { ecmaVersion: options.ecmaVersion || DEFAULT_ECMA_VERSION }));
        }
            ...
            result = parseScriptElement(scripts[0], locationCalculator, Object.assign(Object.assign({}, options), { parser: scriptParser }));
        }
