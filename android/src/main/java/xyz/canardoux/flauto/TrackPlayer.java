	...
	implements MethodCallHandler
{
	...
	public static void attachTrackPlayer (
		Context ctx,
	                         ...
	                         )
	{
		...
		switch ( call.method )
		{
			...
			{
				theTrackPlayer = new TrackPlayer ();
				theTrackPlayer.initializeTrackPlayer ( call, result );
				result.success ( true );
			}
			break;

			case "releaseTrackPlayer":
			{
				theTrackPlayer.releaseTrackPlayer ( call, result );
				result.success ( true );
			}
			break;
			...
			case "startPlayerFromTrack":
				...
				theTrackPlayer.startPlayerFromTrack ( track, canSkipForward, canSkipBackward, result );
				break;


			case "stopPlayer":
				theTrackPlayer.stopPlayer ( result );
				break;
			case "pausePlayer":
				theTrackPlayer.pausePlayer ( result );
				break;
			case "resumePlayer":
				theTrackPlayer.resumePlayer ( result );
				break;
			case "seekToPlayer":
				int sec = call.argument ( "sec" );
				theTrackPlayer.seekToPlayer ( sec, result );
				break;
			case "setVolume":
				double volume = call.argument ( "volume" );
				theTrackPlayer.setVolume ( volume, result );
				break;
			case "setSubscriptionDuration":
				...
				theTrackPlayer.setSubscriptionDuration ( duration, result );
				break;
	...
	extends FlautoPlayer
{
	...
	void releaseTrackPlayer (
		final MethodCall call,
		final Result result
	                        )
	{
		super.releaseFlautoPlayer ( call, result );
	...
	{
		return TrackPlayerPlugin.channel;
	}

	public void startPlayerFromTrack (
		final Track track,
...
//-------------------------------------------------------------------------------------------------------------------------------
		...
		implements Callable<Void>
	{
		...
		MediaPlayerConnectionListener (
			Result result,
			boolean isSuccessfulCallback
		                              )
		{
			mResult               = result;
		...
		implements Callable<Void>
	{
			...
			Exception
		{
			...
			{
				getChannel ().invokeMethod ( "skipForward", null );
			} else
			{
				getChannel ().invokeMethod ( "skipBackward", null );
			}
		...
		implements Function<Integer, Void>
	{
		...
		{
			getChannel ().invokeMethod ( "updatePlaybackState", newState );
			return null;
		}
	}
