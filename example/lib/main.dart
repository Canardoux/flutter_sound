...
class _MyAppState extends State<MyApp> {
  ...
  FlautoPlayer playerModule = FlautoPlayer();
  FlautoRecorder recorderModule = FlautoRecorder();
  FlautoPlayer playerModule_2 = FlautoPlayer(); // Used if REENTRANCE_CONCURENCY
  FlautoRecorder recorderModule_2 = FlautoRecorder(); // Used if REENTRANCE_CONCURENCY
  ...
  t_AUDIO_STATE get audioState => playerModule.isPlaying() ? t_AUDIO_STATE.IS_PLAYING : playerModule.isPaused() ? t_AUDIO_STATE.IS_PAUSED : recorderModule.isRecording() ? t_AUDIO_STATE.IS_RECORDING : t_AUDIO_STATE.IS_STOPPED;

  Future<void> _initializeExample(FlautoPlayer module) async {
    playerModule = module;
  ...
  Future<void> startPlayer() async {
    try {
      ...
      _addListeners();
      if (REENTRANCE_CONCURENCY && _media != t_MEDIA.STREAM) {
